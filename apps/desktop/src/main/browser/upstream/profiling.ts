...
import type { BrowserFiles } from "./files.ts";
import { analyzeCpu, analyzeTrace, parseHeap } from "./analysis.ts";
    ...
    try {
      ...
      return Schema.decodeUnknownSync(Schema.fromJsonString(Schema.Unknown))(
        (file.name.endsWith(".gz")
          ? gunzipSync(data, { maxOutputLength: 128 * 1024 * 1024 })
          : data
        ).toString("utf8"),
      );
    } catch (error) {
            ...
            try {
              ...
              const raw = Schema.decodeUnknownSync(
                Schema.fromJsonString(
                  Schema.Struct({
                    traceEvents: Schema.Array(Schema.Record(Schema.String, Schema.Json)),
                  }),
                ),
              ...
              return {
                ...
                incomplete:
                  ...
                  contents.getOSProcessId() !== owner.pid,
              };
            } finally {
