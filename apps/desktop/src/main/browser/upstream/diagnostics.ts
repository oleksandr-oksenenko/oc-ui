...
export function createDiagnostics(cdp: Cdp) {
  ...
  cdp.on("Runtime.consoleAPICalled", (event) => {
    ...
    add(
      ...
      source
        ? {
            url: source.url.slice(0, Browser.MAX_TEXT),
  ...
  cdp.on("Log.entryAdded", (event) => {
    ...
    add(
      ...
      entry.url
        ? {
            url: entry.url.slice(0, Browser.MAX_TEXT),
            line: (entry.lineNumber ?? 0) + 1,
            column: 1,
          }
        : undefined,
    );
  ...
  cdp.on("Runtime.exceptionThrown", (event) => {
    ...
    add(
      ...
      event.timestamp,
      error.url
        ? { url: error.url, line: error.lineNumber + 1, column: error.columnNumber + 1 }
        : undefined,
    );
  });
  const begin = (
    ...
    const entry: Request = {
      ...
      request: {
        ...
        postData: request.postData?.slice(0, 20_000),
      },
  ...
  const finish = (key: string, timestamp: number, failure?: string) => {
    ...
    if (!request) return;
    const durationMs =
      request.started === undefined ? 0 : Math.max(0, (timestamp - request.started) * 1000);
    request.info =
      failure === undefined
  ...
  cdp.on("Network.responseReceived", (event, sessionID) => {
    ...
    request.info = {
      ...request.info,
      statusCode: request.info.statusCode ?? event.response.status,
    };
  ...
  cdp.on("Network.webSocketClosed", (event, sessionID) => {
    const key = `${sessionID ?? ""}:${event.requestId}`;
    if (latest(key)?.info.state === "pending")
      finish(key, event.timestamp, "Closed before the handshake completed");
  });
  ...
  return {
    ...
    console(input: Extract<Browser.Action, { type: "console" }>) {
      const matching = messages.filter(
        (entry) => levels.indexOf(entry.level) >= levels.indexOf(input.level ?? "info"),
      );
