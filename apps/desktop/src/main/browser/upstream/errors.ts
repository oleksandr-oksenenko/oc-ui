...
export function protocolError(method: string, error: unknown) {
  ...
  const recovery =
    method === "DOM.setFileInputFiles" && /not.*file input/i.test(detail)
      ...
      : /(?:node|object).*(?:not found|not exist)|(?:find|resolve).*(?:node|object)|detached/i.test(
            detail,
          )
...
export function browserFailure(
  ...
  error: unknown,
): Extract<Browser.Outcome, { type: "failure" }> {
          ...
          : network === "ERR_ABORTED"
            ...
            : network
              ...
              : action.type === "screenshot" &&
                  /UnknownVizError|capture.*(?:failed|unavailable)/i.test(detail)
  ...
  return {
    ...
    message:
      `browser.${action.type} failed. ${hint ? `${hint} Details: ${detail.slice(0, 400)}` : detail}`.slice(
        0,
        2_048,
      ),
  };
