...
- Start with that guide's existing commands; `pnpm verify:browser` supplies an isolated browser/server inspection session. Keep one verification owner, save full logs with bounded output, and wait for completion instead of repeatedly polling. Reuse passing evidence until a relevant change or failure requires another run.
