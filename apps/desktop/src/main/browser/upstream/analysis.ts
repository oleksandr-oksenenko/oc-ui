...
export function analyzeTrace(value: unknown, limit = 100) {
  ...
  trace.traceEvents.forEach((event) => {
    ...
    const item = events.get(event.name) ?? {
      name: event.name.slice(0, 2_048),
      count: 0,
      totalMs: 0,
      maxMs: 0,
    };
  ...
  return {
    metrics: [
      ...
      {
        name: "longTaskBlocking",
        value: longTasks.reduce((sum, duration) => sum + duration - 50, 0),
        unit: "ms",
      },
    ],
...
export function parseHeap(value: unknown) {
  ...
  // Field indexes and record widths are validated above; traversal offsets stay inside each record.
  ...
  for (let offset = indexes.to; offset < heap.edges.length; offset += edgeWidth) {
    ...
    if (!Number.isSafeInteger(to) || to < 0 || to >= heap.nodes.length || to % width)
      throw malformedHeap();
  }
  return {
    summary(limit = 100) {
    ...
    query(name = "", limit = 100) {
      ...
      return {
        nodes: found.sort((a, b) => b.selfBytes - a.selfBytes).slice(0, limit),
        truncated: found.length > limit,
      };
    },
    object(id: number, limit = 100) {
      const target =
        heap.nodes.findIndex((value, index) => index % width === indexes.id && value === id) -
        indexes.id;
      if (target < 0)
        throw new Error(
      ...
      for (let offset = 0; offset < heap.nodes.length; offset += width) {
        ...
        for (let index = 0; index < count; index++, edgeOffset += edgeWidth) {
          ...
          const name = (
            type === "element" || type === "hidden" ? String(raw) : (heap.strings[raw] ?? "")
          ).slice(0, 100_000);
          if (offset === target) {
