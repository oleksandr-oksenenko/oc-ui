...
export function htmlToPlainText(html: string): string {
  ...
  const render = (node: Node): string => {
    ...
    if (tag === "BR") return "\n";
    const inner = Array.from(node.childNodes, render).join("");
    return BLOCK_TAGS.has(tag) ? `\n\n${inner}\n\n` : inner;
  };
  const raw = Array.from(dom.body.childNodes, render).join("");
  return raw.replace(/\n{3,}/g, "\n\n").trim();
}
...
export type ClipboardRead = {
  ...
  /** Bounded `text/html`, absent when the format is missing or over the bound. */
  readonly html?: string;
  /** `text/html` was present but exceeded {@link MAX_HTML_INSPECTION_UNITS}. */
 ...
 * not read when it cannot win: the literal/code routes ignore it, and text
 * over the routing threshold already becomes an attachment. A blank plain
 * flavor falls back to the HTML flavor's derived text so the router and the
 * attachment owner never see an empty payload that carries content.
 */
/**
 * Reads the text and HTML flavors with the routing context. The HTML flavor is
 * not read when it cannot win: a code-block context ignores it, and text over
 * the routing threshold already becomes an attachment. A blank plain flavor
 * falls back to the HTML flavor's derived text so the router and the
 * attachment owner never see an empty payload that carries content.
 */
export function readClipboardText(
  ...
  if (wantsHtml && hasFlavor(data, "text/html")) {
    ...
    if (raw !== "") {
      if (raw.length > MAX_HTML_INSPECTION_UNITS) htmlOversize = true;
      else html = raw;
    }
  }
